...
let tvimage = card.getElementsByClassName("main-card-img");
    tvimage[0].src = item.imgUrl;

let cardTitle = card.getElementsByClassName("card-title");
...
let reviews = card.getElementsByClassName("card-footer-body-reviews");
    reviews[0].textContent = item.orderInfo.reviews;
  
let orders = card.getElementsByClassName("card-footer-footer-orders");
    orders[0].textContent = Math.floor(Math.random() * 10000) + 1;

let bgInfo = cardInfo.querySelector(".card-info-background");
bgInfo.addEventListener("click", ()=> remove()); 
...
function showInfoBox (item){
    let img = cardInfo.querySelector(".main-card-img");
    img.src = item.imgUrl;

    let cardTitle = cardInfo.querySelector(".card-title");
    cardTitle.textContent = item.name;

    let reviews = cardInfo.querySelector(".card-footer-body-reviews");
    reviews.textContent = item.orderInfo.reviews;

    let price = cardInfo.querySelector(".price");
    price.textContent = item.price;
    
    let color = cardInfo.querySelector(".color");
    color.textContent = item.color;

    let os = cardInfo.querySelector(".operation-system");
    os.textContent = item.os;
    
    let chip = cardInfo.querySelector(".chip");
    chip.textContent = item.chip.name;

    let deviceHeight = cardInfo.querySelector(".device-height");
    deviceHeight.textContent = item.size.height;

    let deviceWidth = cardInfo.querySelector(".device-width");
    deviceWidth.textContent = item.size.width;

    let deviceDepth = cardInfo.querySelector(".device-depth");
    deviceDepth.textContent = item.size.depth;

    let deviceWeight = cardInfo.querySelector(".device-weight");
    deviceWeight.textContent = item.size.weight;

    let count = cardInfo.querySelector(".count-in-stock");
...
// let click = document.querySelector("#display5-7");
// click.addEventListener("click", cardRefresh);

// function cardRefresh (){
//     let filter = items.filter(item => (item.display >= 5 && item.display <= 7))
    
      
//     if (filter) {
//         itemsContainer.innerHTML = "";
//         // newElement.innerHTML = card.innerHTML;
//         itemsContainer.newElement.filter;
...
// function filter() {
//     if(filterCheck.onchange){
//         console.log(items.filter(item => (item.display <= 2 && item.display >= 5)));
//     }
//     filter();
// };
// filterCheck.onchange = function() {
//     items.filter(item => (item.display <= 2 && item.display >= 5));
//     itemsContainer.innerHTML = "";
...
/* Filter section Открытие фильтров */

document.querySelector(".filter-block").addEventListener("click", viewBlock);

const windowfilter =  document.querySelector(".window-filter");
const containerElementCard = document.querySelector(".container-element-card");

function viewBlock(){

    if(windowfilter.style.display !== "block"){
       
    windowfilter.style.display = "block";
    containerElementCard.style.width = "70" + "%";
    containerElementCard.style.justifyContent = "space-between";
   }

   else {
    windowfilter.style.display = "none";
    containerElementCard.style.width = "100" + "%";
   }
};
  


document.querySelector("#price").addEventListener("click", viewPrice);

const windowPrice =  document.querySelector(".window-filter-price-body");

function viewPrice(){

    if(windowPrice.style.display !== "block"){
       
        windowPrice.style.display = "block";    
   }
...
document.querySelector("#color").addEventListener("click", viewColor);

const windowColor =  document.querySelector(".window-filter-color-body");

function viewColor(){

    if(windowColor.style.display == "flex"){

        windowColor.style.display = "none";
   }

   else {
    windowColor.style.display = "flex";    
   }
};

document.querySelector("#memory").addEventListener("click", viewMemory);

const windowMemory =  document.querySelector(".window-filter-memory-body");

function viewMemory(){

    if(windowMemory.style.display == "flex"){
       
        windowMemory.style.display = "none";        
   }

   else {
    windowMemory.style.display = "flex";
    
   }
};


document.querySelector("#os").addEventListener("click", viewOS);

const windowOs =  document.querySelector(".window-filter-os-body");

function viewOS(){

    if(windowOs.style.display == "flex"){
       
        windowOs.style.display = "none";       
   }

   else {
    windowOs.style.display = "flex";
    
   }
};


document.querySelector("#display").addEventListener("click", viewDisplay);

const windowDisplay =  document.querySelector(".window-filter-display-body");

function viewDisplay(){

    if(windowDisplay.style.display == "flex"){
